...
do
  echo "seed is ${seed}:"
  CUDA_VISIBLE_DEVICES=0 python ../train/train_lattice.py --env_dim ${env_dim} --algorithm_name ${algo} --log_interval 1 --num_env_steps 60000 \
      --env_name ${env} --scenario_name ${scenario} --user_name 'Anonymous' --episode_length 25 --cuda --n_rollout_threads 4 --use_linear_lr_decay\
      ...
      --freq_type 'step' --train_freq 8 --prioritized_replay_alpha 0.6 --buffer_size 10000 --memory_alpha 0.6 --save_interval 0  \
