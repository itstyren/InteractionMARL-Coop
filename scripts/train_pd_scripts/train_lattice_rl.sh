...
do
  ...
  CUDA_VISIBLE_DEVICES=0 python ../train/train_lattice.py --env_dim ${env_dim} --algorithm_name ${algo} --log_interval 1 --num_env_steps 5000 \
      --env_name ${env} --scenario_name ${scenario} --user_name 'tyren' --episode_length 25 --cuda --n_rollout_threads 2 --use_linear_lr_decay\
      ...
      --experiment_name ${exp} --use_render --use_wandb --lr 0.1 --video_interval 1 --use_linear_beta_decay --replay_scheme 'prioritized' --learning_starts 50 \
