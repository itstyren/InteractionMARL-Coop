...
do
  ...
  CUDA_VISIBLE_DEVICES=2 python ../train/train_lattice.py --env_dim ${env_dim} --algorithm_name ${algo} --log_interval 1 --num_env_steps 10000 \
      --env_name ${env} --scenario_name ${scenario} --user_name 'tyren' --episode_length 20 --cuda --n_rollout_threads 8 --use_linear_lr_decay\
      ...
      --freq_type 'step' --train_freq 32 --prioritized_replay_alpha 0.6 --buffer_size 8000 --memory_alpha 0.6 --save_interval 0  \
