...
do
  ...
  CUDA_VISIBLE_DEVICES=2 python ../train/train_lattice.py --env_dim ${env_dim} --algorithm_name ${algo} --log_interval 1 --num_env_steps 5000 \
      ...
      --experiment_name ${exp} --use_render  --lr 0.1 --video_interval 10 --use_linear_beta_decay --replay_scheme 'prioritized' --learning_starts 100 \
      --freq_type 'step' --train_freq 8 --prioritized_replay_alpha 0.6 --buffer_size 5000 --memory_alpha 0 --save_interval 0  \
